...
import { TDataTransactionEntry, IWithId, TSignedTransaction, TTransaction } from '@waves/ts-types';
 ...
 * @param after
 */
export function fetchStateChangesByAddress(base: string, address: string, limit: number, after?: string): Promise<Array<TSignedTransaction<TTransaction<TLong>> & IWithId & IWithStateChanges>> {
    return request({
        base,
 ...
 * @param txId
 */
export function fetchStateChangesByTxId(base: string, txId: string ): Promise<TSignedTransaction<TTransaction<TLong>> & IWithId & IWithStateChanges> {
    return request({
        base,
